...
X_space = np.linspace(0, 10, num=100).reshape(-1,1)
...
X_space_stacked = X_space
W_opt = None
"*** YOUR CODE HERE ***"
ones=np.ones_like(X_space)
X_space_stacked=np.hstack((ones,X_space))

W_opt=np.linalg.solve(X_space_stacked.T.dot( X_space_stacked), X_space_stacked.T.dot( y_space_rand))
"*** END YOUR CODE HERE ***"
