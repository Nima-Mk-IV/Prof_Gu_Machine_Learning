...
def linreg(X, y, reg=0.0):
	...
	eye[0, 0] = 0
	W_opt=np.linalg.solve(X.T.dot(X)+reg*eye, X.T.dot(y))
...
def find_RMSE(W, X, y):
	...
	y_diff_sq_mean=np.mean(y_diff_sq)
	RMSE=np.sqrt(y_diff_sq_mean)
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	W_list = []
	"*** YOUR CODE HERE ***"
	reg_list = np.random.uniform(0.0, 200.0, 200)
	reg_list.sort()
	W_list = [linreg(X_train, y_train, reg = reg) for reg in reg_list]
	RMSE_list=[find_RMSE(Wi, X_val, y_val) for Wi in W_list]
...
def linreg_no_bias(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	m = X.shape[0]
	Agg = X.T.dot(np.eye(m) - np.ones(m) / m)
	W_opt = np.linalg.solve(Agg.dot(X) + reg * np.eye(Agg.shape[0]), Agg.dot(y))
	b_opt = sum((y - X.dot(W_opt))) / m
	"*** END YOUR CODE HERE ***"
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		obj_train=obj_train+[train_rmse]
		obj_val=obj_val+[find_RMSE(W, X_val, y_val)]
			
		W_grad = ((X_train.T.dot(X_train) + reg*np.eye(n)).dot(W) + X_train.T.dot((b - y_train))) / m_train
		b_grad = (sum(X_train.dot(W))-sum(y_train)+b*m_train)/m_train
