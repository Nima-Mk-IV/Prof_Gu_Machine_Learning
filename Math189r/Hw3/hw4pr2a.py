...
def grad_logreg(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	grad= X.T.dot(sigmoid(X.dot(W)) - y) + reg * W
...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = sigmoid(X.dot(W))
	nll = -sum(np.multiply(y, np.log(mu)) + np.multiply((1.0 - y), np.log(1.0 - mu))) + reg / 2 * np.linalg.norm(W) ** 2
	nll=nll.item(0)
	"*** END YOUR CODE HERE ***"
	return nll
...
def newton_step(X, y, W, reg=0.0):
	...
	hessian = X.T.dot(diag.dot(X)) + reg * np.eye(X.shape[1])
...
def get_description(X, y, W):
	...
	for index in range(m):
		actual, pred = y[index], y_pred[index]
		if actual == pred:
			count_a += 1
		if actual == 1:
			total_r += 1
		...
		if pred == 1:
			...
			if actual == 1:
				count_p += 1
	accuracy = 1.0 * count_a / m
	precision = 1.0 * count_p / total_p
	recall = 1.0 * count_r / total_r
