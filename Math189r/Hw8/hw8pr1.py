...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
    ...
    while iter_num < max_iter:
        ...
        for i in range(m):
            data = X[i, :]
            difference = data - clusters
            current_label = np.argsort(np.linalg.norm(difference, axis=1)).item(0)
            label[i] = current_label
        
        
        for center in range(k):
            index = np.where(label == center)[0]
            if len(index) > 0:
                clusters[center, :] = X[index].mean(axis=0)
